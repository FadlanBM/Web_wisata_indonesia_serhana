...
@section('main')
<br>
        <p class="title" style="font-size: 25px; font-style: inherit;">Halaman Tambah data Seni Tulis</p>
...
<table class="table table-stripped table-hover">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        @if ($item->foto)
            <img style="max-width: 50px; max-height: 50px" src="{{url('assets/img/foto_seni_tulis').'/'.$item->foto}}" alt="">
        @endif
      </td>
      <td>{{$item->name}}</td>
      <td>{{$item->asal}}</td>
      ...
      <td>
          <a href='{{route('seni_tulis.edit',$item->id)}}' class="btn btn-sm btn-warning">Edit</a>
          <form onsubmit="return confirm('Yakin Mau hapus data ini?')" action="{{route('seni_tulis.destroy',$item->id)}}" method="POST" class="d-inline">
            @csrf
            @method('DELETE')
  ...
  <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="{{route('seni_tulis.store')}}" method="POST" enctype="multipart/form-data">
          @csrf
...
<input type="text" class="form-control" id="judul" name="judul" value="{{Session::get('judul')}}" placeholder="Masukkan Thumbnail Tempat Wisata">
    ...
    <div class="form-group">
    <label for="buatan">Pembuat</label>
    <input type="text" class="form-control" id="buatan" name="buatan" value="{{Session::get('buatan')}}" placeholder="Masukkan Nama Pembuat Seni Tulis">
  </div>
    <div class="form-group">
